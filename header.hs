...
getHeaders bstr | B.null bstr = []
                | otherwise = let header = findNextHeader bstr
                                  next = B.drop (frameLength header) bstr
...
seconds mi = round (secondsPerFrame * l)
  where l = fromIntegral $ length (valid mi)

minutes mi = div (seconds mi) 60
secs mi = (mod) (seconds mi) 60

biteRates mi = if const then show x else "VBR"
  where getbr (Valid {bitrate=(Br i)}) = i
        (x:xs) = map getbr (valid mi)
        const = and (map (\y->x==y) xs)

getLayer = getlayer . head . valid
  where getlayer = show . layer

getVersion = getversion . head . valid
  where getversion = show . version

getCopyright = getcopyright . head . valid
  where getcopyright = yesno . copy

getCrc = getCrc . head . valid
  where getCrc = yesno . protected

getMode = getmode . head . valid
  where getmode = show . mode

getFrequency = getfrequency . head . valid
  where getfrequency = show . frequency

getFrequencyK = getfrequency . head . valid
  where getfrequency  = g . frequency
        g (F i) = show (i `div` 1000)
        g f = show f

getEmphasis = getEmphasis . head . valid
  where getEmphasis = show . emphasis

getOriginality = getOriginality . head . valid
  where getOriginality = show . original
        
getPadded = getpad . head . valid
...
getValidFrames = show . length . valid 

readMP3Info::Handle->IO MP3Info
readMP3Info h = do
  s <- B.hGetContents h
  return (getHeaders s)